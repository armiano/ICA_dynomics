...
for i,j in worker_tasks[rank]:
    ...
    S2_matrix =S2.to_numpy()
        #for column in S1:
        #S1_matrix[column] = pd.to_numeric(S1[column], downcast="float", errors='coerce').fillna(0).astype(float)
        # S2_matrix[column] = pd.to_numeric(S2[column], downcast="float", errors='coerce').fillna(0).astype(float)
    
        # S1_matrix[column] = pd.to_numeric(S1[column], downcast="float", errors='coerce').fillna(0).astype(float)
        #  for column in S2:
        # S2_matrix[column] = pd.to_numeric(S2[column], downcast="float", errors='coerce').fillna(0).astype(float)
    dist = abs(np.dot(S1_matrix.T,S2_matrix))
