...
if [ "$VERBOSE" = true ]; then
    ...
    echo "" > $LOGFILE
    $HOME/opt/usr/local/bin/mpiexec -n $CORES python -u random_restart_ica.py -f $FILE -i $ITER -o $OUTDIR -t $TOL >> $LOGFILE 2>&1
    $HOME/opt/usr/local/bin/mpiexec -n $CORES python -u compute_distance.py -i $ITER -o $OUTDIR >> $LOGFILE 2>&1
    $HOME/opt/usr/local/bin/mpiexec -n $CORES python -u cluster_components.py -i $ITER -o $OUTDIR >> $LOGFILE 2>&1
fi
