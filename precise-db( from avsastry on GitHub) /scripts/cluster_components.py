...
if rank == 0:
    ...
    for i in range(nWorkers):
        ...
        S_part_M =pd.DataFrame()
        A_part_M =pd.DataFrame()
        for column in S_part:
             S_part_M[column] = pd.to_numeric(S_part[column], downcast="float", errors='coerce').fillna(0).astype(float)
        for column in A_part:
             A_part_M[column] = pd.to_numeric(A_part[column], downcast="float", errors='coerce').fillna(0).astype(float)
                
        S_part_M.columns = range(S_part.shape[1])
        A_part_M.columns = range(A_part.shape[1])
        for i,lab in enumerate(proc_labels):
            if lab != -1:
                S_bins[lab].append(S_part_M[i])
                A_bins[lab].append(A_part_M[i])
